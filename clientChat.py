...
class LeetcodeClient:
    ...
    async def fetch_completion(self, session, messages):
        data = {
            "model": self.model,
            "messages": messages,
        ...
        async with session.post(self.base_url + "/v1/chat/completions", headers=self.api_headers, json=data) as response:
    ...
    def generate_solutions(self): 
        ...
        solution_codes = [extract_code_sections(response["choices"][0]["message"]["content"])[-1] for response in responses]
    ...
    def generate_debugs(self):
        ...
        for i, solution in enumerate(self.solutions):
            solution["code"] = solution_codes[i]
        self.debug_message(start_time, responses)
    ...
    def test_solution(self, solution):
        ...
        try: 
            process = subprocess.run(['python', 'test.py'], capture_output=True, text=True, timeout=10)
...
if __name__ == "__main__": 
    ...
    client = LeetcodeClient(problem, method_header, config["OPENAI_ENDPOINT"], config["OPENAI_API_KEY"], config["MODEL"], config["MAX_TOKENS"], max_solutions=15)
    client.run()